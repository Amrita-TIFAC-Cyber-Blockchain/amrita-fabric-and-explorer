...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"Org3MSP":.[1]}}}}}' config.json ./organizations/peerOrganizations/org3.amrita.edu/org3.json > modified_config.json
{ set +x; } 2>/dev/null
...
setGlobals 2
set -x
peer channel update -f org3_update_in_envelope.pb -c ${CHANNEL_NAME} -o orderer.amrita.edu:7050 --ordererTLSHostnameOverride orderer.amrita.edu --tls --cafile "$ORDERER_CA"
{ set +x; } 2>/dev/null
